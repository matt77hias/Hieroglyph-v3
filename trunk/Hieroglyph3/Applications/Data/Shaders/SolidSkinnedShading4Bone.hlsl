...
VS_OUTPUT VSMAIN( in VS_INPUT input )
{
	...
	output.normal  = (mul( float4( input.normal, 0.0f ), SkinNormalMatrices[input.bone.x] ) * input.weights.x).xyz;
	output.normal += (mul( float4( input.normal, 0.0f ), SkinNormalMatrices[input.bone.y] ) * input.weights.y).xyz;
	output.normal += (mul( float4( input.normal, 0.0f ), SkinNormalMatrices[input.bone.z] ) * input.weights.z).xyz;
	output.normal += (mul( float4( input.normal, 0.0f ), SkinNormalMatrices[input.bone.w] ) * input.weights.w).xyz;
